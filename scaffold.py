...
def _write_with_frontmatter(dest: pathlib.Path, meta: dict, body: str) -> None:
    dest.write_text(f"---\n{yaml.dump(meta, default_flow_style=False, allow_unicode=True, sort_keys=False, width=float('inf'))}---\n\n{body}")
...
def scaffold(
    ...
    paths_cfg: dict = yaml.safe_load((config_dir / "paths.yml").read_text())
    agents_cfg: dict = yaml.safe_load((config_dir / "agents.yml").read_text())
    skills_cfg: dict = yaml.safe_load((config_dir / "skills.yml").read_text())
